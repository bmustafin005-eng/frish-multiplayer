...
function botUsePodlozhki(r,i){let changed=true;while(changed){changed=false;outer:for(let hi=0;hi<r.hands[i].length;hi++){const c=r.hands[i][hi];if(isJ(c))continue;for(const m of r.melds){const front=[c,...m.cards],back=[...m.cards,c];const vf=validateMeldCanonical(r,front,'NORMAL'),vb=validateMeldCanonical(r,back,'NORMAL');if(vf.valid||vb.valid){m.cards=vf.valid?front:back;m.validation=vf.valid?vf:vb;r.hands[i].splice(hi,1);addLog(r,`${r.players[i].name} подложил ${cardText(c)}`);changed=true;break outer}}}}}
...
io.on('connection',socket=>{
 ...
 socket.on('extendMeld',({meldId,ids})=>{const[r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn)return;const m=r.melds.find(x=>String(x.id)===String(meldId));if(!m)return;ids=(ids||[]).map(Number);const picked=[];for(const id of ids){const c=r.hands[i].find(x=>x.id===id);if(c)picked.push(c)}if(!picked.length)return;const front=[...picked,...m.cards],back=[...m.cards,...picked];const vf=validateMeldCanonical(r,front,r.allDeclared[i]?'ALL_FINISH':'NORMAL'),vb=validateMeldCanonical(r,back,r.allDeclared[i]?'ALL_FINISH':'NORMAL');let trial=null,v=null;if(vf.valid){trial=front;v=vf}else if(vb.valid){trial=back;v=vb}if(!trial)return socket.emit('errorMsg','Эти карты нельзя подложить к выбранному терцу.');const set=new Set(picked.map(c=>c.id));r.hands[i]=r.hands[i].filter(c=>!set.has(c.id));m.cards=trial;m.validation=v;addLog(r,`${r.players[i].name} подложил ${picked.map(cardText).join(' ')}`);emit(r);});
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v0.9.1 HAND BOTTOM</title><style>
...
#hand .card{width:39px!important;height:58px!important;min-width:39px!important;margin:0!important}
...
server.listen(PORT,()=>console.log('FRISH v0.9.1 HAND BOTTOM on',PORT));
