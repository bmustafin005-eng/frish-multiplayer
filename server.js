...
function txn(r,i,type,cards,extra={}){if(!r.allDeclared[i])return;if(!r.allTxn[i])r.allTxn[i]=[];r.allTxn[i].push({type,cards:(cards||[]).map(c=>typeof c==='object'?c.id:c),at:Date.now(),...extra})}
function autoCard(r,i){if(r.hands[i].length===1&&!r.cardSaid[i]){r.cardSaid[i]=true;if(r.cardWarning[i])r.cardWarning[i].said=true;addLog(r,`${r.players[i].name} — КАРТА`);notice(r,`${r.players[i].name} — КАРТА`,5000);return true}return false}
...
function botFindRun(r,i){
 let cand=botCandidateMelds(r,i);if(r.botDifficulty==='EASY')cand=cand.slice(0,10);else if(r.botDifficulty==='MEDIUM')cand=cand.slice(0,28);let best=null;
...
function botUsePodlozhki(r,i){
 ...
 while(changed){changed=false;
  outer:for(let hi=0;hi<r.hands[i].length&&r.hands[i].length>1;hi++){const c=r.hands[i][hi];if(isJ(c))continue;
...
function botTurn(r){
 ...
 if(i===r.dealer&&r.hands[i].length===15&&!r.discard.length){let k=r.hands[i].findIndex(c=>!isJ(c));if(k<0)k=0;r.discard.push(r.hands[i].splice(k,1)[0]);addLog(r,`${r.players[i].name} сбросил карту`);autoCard(r,i);return endTurn(r)}
 if(!r.stock.length){startStockRebuild(r);return}
 const botDrawn=r.stock.pop();r.hands[i].push(botDrawn);addLog(r,`${r.players[i].name} взял карту из колоды`);if(r.ran[i]&&isJ(botDrawn)){addLog(r,`${r.players[i].name} потащил JOKER`);notice(r,`${r.players[i].name} ПОТАЩИЛ JOKER`,5000)}
 ...
 const fin=CAN_FINISH_NOW(r,i);if(fin){
   ...
   r.allFinisher=i;r.phase='finish_report';r.allDeadline=Date.now()+10000;emit(r);
   r.reportTimer=setTimeout(()=>{if(r.phase!=='finish_report'||r.allFinisher!==i)return;finishOrdinaryDeal(r,i);addLog(r,`Раздача №${r.dealNo} завершена`);r.phase='deal_end';emit(r);setTimeout(()=>{r.dealNo++;r.mult=[1,6,11,16].includes(r.dealNo)?2:1;startDeal(r,false)},3500)},10000);return;
 }
 ...
 if(r.hands[i].length===0)return applyZastrel(r,i,'бот попытался продолжить игру с 0 карт');autoCard(r,i);
 endTurn(r);
}
...
io.on('connection',socket=>{
 ...
 if(r.allDeclared[i]&&r.pairFinish[i]&&r.pairFinish[i].count===7&&r.hands[i].length===1){const c=r.hands[i].splice(k,1)[0];r.discard.push(c);txn(r,i,'DISCARD',[c],{final:r.allDeclared[i]&&r.hands[i].length===0});if(r.hands[i].length===0&&!r.allDeclared[i])return applyZastrel(r,i,'закончил без объявления ВСЕ');r.takenDiscardId[i]=null;r.reclaimedJokerId[i]=null;return beginFinishReport(r,i,'pairs');}
 if(r.allDeclared[i]&&r.hands[i].length!==1)return applyZastrel(r,i,'неправильное ВСЕ');
 const c=r.hands[i].splice(k,1)[0];r.discard.push(c);txn(r,i,'DISCARD',[c],{final:r.allDeclared[i]&&r.hands[i].length===0});
 ...
 socket.on('meld',ids=>{const [r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn||(!r.drawn&&!(i===r.dealer&&r.hands[i].length===15&&!r.discard.length)))return;
   ...
   r.hands[i]=r.hands[i].filter(c=>!ids.includes(c.id));txn(r,i,'MELD',cs);
   ...
   if(!r.players[i].bot&&r.hands[i].length===0)return applyZastrel(r,i,'0 карт без законного финального сброса');
   emit(r)
 });
 // Подложка: JOKER считается полноценной добавленной позицией. После него длина может продолжаться,
 // но суммарно за этот ход разрешено не более двух новых карт с каждой стороны терца.
 socket.on('extendMeld',({meldId,ids})=>{const[r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn)return;
   ...
   if(r.takenDiscardId[i]!=null&&set.has(r.takenDiscardId[i]))r.takenDiscardId[i]=null;if(r.reclaimedJokerId[i]!=null&&set.has(r.reclaimedJokerId[i])){addLog(r,`${r.players[i].name} использовал забранный JOKER как подложку`);r.reclaimedJokerId[i]=null;}
   m.cards=trial;m.validation=v;m.extTurn[side]+=picked.length;txn(r,i,'PODLOZHKA',picked,{meldId:m.id,side});
   if(v.valid&&v.type==='SAME_RANK'&&trial.length===4&&m.buryOnTurnOf==null)m.buryOnTurnOf=i;
   addLog(r,`${r.players[i].name} подложил ${picked.map(cardText).join(' ')}`);if(!r.players[i].bot&&r.hands[i].length===0)return applyZastrel(r,i,'0 карт без законного финального сброса');emit(r);
 });
 socket.on('declareAll',()=>{const[r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn||r.allDeclared[i])return;r.allFromHand[i]=!r.ran[i];r.allDeclared[i]=true;r.allTxn[i]=[{type:'ALL_START',cards:r.hands[i].map(c=>c.id),at:Date.now()}];addLog(r,`${r.players[i].name} — ВСЕ`);notice(r,`${r.players[i].name} — ВСЕ`,5000);emit(r); });
 ...
 const set=new Set(ids);r.hands[i]=r.hands[i].filter(c=>!set.has(c.id));r.hands[i].push(jok);r.reclaimedJokerId[i]=jok.id;txn(r,i,'RECLAIM_JOKER',add,{jokerId:jok.id,meldId:m.id});
 ...
 socket.on('setBotDifficulty',v=>{const[r,i]=findRoomBySocket(socket);if(r&&i===0&&!r.started){const x=String(v||'MEDIUM').toUpperCase();if(['EASY','MEDIUM','HARD'].includes(x))r.botDifficulty=x;emit(r)}});
...
function finishSuit(){if(sel.size!==14)return uiError('Выберите 14 карт одной масти; один JOKER может заменить недостающую карту.');socket.emit('finishSuit',orderedHand().filter(c=>sel.has(c.id)).map(c=>c.id));sel.clear()}
function startReclaim(){if(!(S.ran&&S.ran[S.myIndex])&&!(S.allDeclared&&S.allDeclared[S.myIndex]))return;if(sel.size!==2)return uiError('Выберите две недостающие карты.');reclaimMode=true;reclaimTarget=null;render()}
...
function layMeld(){if(sel.size<3)return uiError('Выберите минимум 3 карты.');socket.emit('meld',orderedHand().filter(c=>sel.has(c.id)).map(c=>c.id));sel.clear()}function discardSelected(){if(sel.size!==1)return uiError('Выберите одну карту.');socket.emit('discard',[...sel][0]);sel.clear()}function startExtend(){if(!((S.ran&&S.ran[S.myIndex])||(S.runReady&&S.runReady[S.myIndex])||(S.allDeclared&&S.allDeclared[S.myIndex])))return;if(!sel.size)return;extendMode=true;extendTarget=null;render()}function confirmExtend(){if(!extendMode||extendTarget==null)return;socket.emit('extendMeld',{meldId:extendTarget,ids:orderedHand().filter(c=>sel.has(c.id)).map(c=>c.id)});sel.clear();extendMode=false;extendTarget=null} 
let handOrder=[];function orderedHand(){if(!S)return[];const m=new Map(S.myHand.map(c=>[c.id,c]));const out=[];handOrder.forEach(id=>{if(m.has(id)){out.push(m.get(id));m.delete(id)}});for(const c of S.myHand){if(m.has(c.id)){out.push(c);m.delete(c.id)}}handOrder=out.map(c=>c.id);return out}function installDrag(){
 ...
 h.querySelectorAll('.card[data-id]').forEach(el=>{
   el.onclick=null;
   el.onpointerdown=e=>{e.preventDefault();dragActive=true;pid=e.pointerId;dragId=Number(el.dataset.id);sx=e.clientX;sy=e.clientY;moved=false;previewIndex=null;try{el.setPointerCapture(pid)}catch(_){}};
   ...
   el.onpointerup=e=>{if(pid!==e.pointerId)return;const z=hit(e.clientX,e.clientY),ids=selectedIds();
     ...
     clearPreview();pid=null;dragId=null;previewIndex=null;dragActive=false;if(pendingState){S=pendingState;pendingState=null}render()
   };
   el.onpointercancel=()=>{clearPreview();pid=null;dragId=null;previewIndex=null;dragActive=false;if(pendingState){S=pendingState;pendingState=null}render()}
 })
