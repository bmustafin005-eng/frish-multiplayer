...
function botUsePodlozhki(r,i){
 let changed=true;
 while(changed){changed=false;outer:for(let hi=0;hi<r.hands[i].length;hi++){const c=r.hands[i][hi];if(isJ(c))continue;for(const m of r.melds){for(const side of ['front','back']){const trial=side==='front'?[c,...m.cards]:[...m.cards,c];const v=validateMeldCanonical(r,trial,'NORMAL');if(v.valid){m.cards=trial;m.validation=v;r.hands[i].splice(hi,1);changed=true;break outer}}}}}
}
...
function botTurn(r){
 ...
 if(r.hands[i].length){let k=r.hands[i].findIndex(c=>!isJ(c));if(k<0)k=0;r.discard.push(r.hands[i].splice(k,1)[0]);addLog(r,`${r.players[i].name} сбросил карту`)}
 endTurn(r);
}
...
io.on('connection',socket=>{
 ...
 socket.on('declareAll',()=>{const[r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn)return;
   if(r.allDeclared[i])return;
   r.allDeclared[i]=true;r.allDeadline=Date.now()+40000;
   addLog(r,`${r.players[i].name} — ВСЕ`);
   emit(r)
 });
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v0.8.9 GAMEPLAY FIXES</title><style>
