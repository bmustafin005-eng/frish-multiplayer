...
function startGame(r){for(let i=0;i<4;i++)if(!r.players[i])r.players[i]={name:NAMES[i],sid:null,bot:true};for(let i=r.players.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[r.players[i],r.players[j]]=[r.players[j],r.players[i]]}r.playerSuits=['♥','♣','♦','♠'];r.started=true;addLog(r,'Масти вытянуты. Рассадка: '+r.players.map((p,i)=>r.playerSuits[i]+' '+p.name).join(' · '));startDeal(r)}
...
function completeCut(r,pos,auto=false){if(r.phase!=='cut')return;clearTimeout(r.cutTimer);pos=Math.max(10,Math.min(r.stock.length-10,Number(pos)||Math.floor(r.stock.length/2)));const cut=r.stock.splice(0,pos);r.svetka=cut.pop();r.stock=[...r.stock,...cut];r.phase='deal';addLog(r,`${r.players[r.cutter].name} ${auto?'снял автоматически':'снял колоду'}. Светка: ${cardText(r.svetka)}`);if(r.svetka&&r.svetka.rank==='2'&&r.svetka.suit==='♠'){const bonus=10*r.mult;r.dealAccum[r.cutter]+=bonus;addLog(r,`Светка 2♠: ${r.players[r.cutter].name} +${bonus}; множитель ×${r.mult} → ×${r.mult+1}`);r.mult+=1;r.dealDisplayMult=r.mult;}for(let i=0;i<4;i++){let n=i===r.dealer?15:14;for(let k=0;k<n;k++)r.hands[i].push(r.stock.pop())}r.phase='turn';r.turn=r.dealer;r.drawn=false;emit(r);beginTurn(r)}
...
function beginTurn(r){buryDue(r);r.turnSerial++;for(let p=0;p<4;p++){const w=r.cardWarning[p];if(w&&!w.said&&r.turnSerial>=w.dueSerial)return applyZastrel(r,p,'не объявлена КАРТА за полный круг')}r.turnMeldPoints[r.turn]=0;r.runReady[r.turn]=false;r.undoSnapshot[r.turn]=null;emit(r);if(r.players[r.turn].bot)setTimeout(()=>botTurn(r),650)}
...
io.on('connection',socket=>{
 ...
 addLog(r,`${r.players[i].name} сбросил ${cardText(c)}`);if(r.hands[i].length===1){r.cardSaid[i]=false;r.cardWarning[i]={dueSerial:r.turnSerial+4,said:false}}else r.cardWarning[i]=null;
 ...
 socket.on('sayCard',()=>{const[r,i]=findRoomBySocket(socket);if(!r)return;if(r.hands[i].length!==1)return socket.emit('errorMsg','КАРТА объявляется, когда в руке осталась ровно одна карта.');if(r.cardSaid[i])return;r.cardSaid[i]=true;if(r.cardWarning[i])r.cardWarning[i].said=true;addLog(r,`${r.players[i].name} — КАРТА`);notice(r,`${r.players[i].name} — КАРТА`,5000)});
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v1.0.0 DIALOG RULES UPDATE</title><style>
...
/* v1.0.0 compact table: keep all laid terets visible without vertical scrolling */
...
function showFinal(){const f=S.finalSettlement,n=S.players.map(p=>p.name);let rows=n.map((x,i)=>'<tr><td>'+S.playerSuits[i]+' '+x+'</td><td>'+f.points[i]+'</td><td>'+(f.net[i]>=0?'+':'')+f.net[i]+'</td></tr>').join('');showModal('<h2>МАТЧ ЗАВЕРШЁН · 16 РАЗДАЧ</h2><p>Ставка: <b>'+f.bet+' монет/очко</b></p><table class="scoreOverlayTable"><tr><th>Игрок</th><th>Итоговые очки</th><th>Расчёт по ставке</th></tr>'+rows+'</table><p><small>Расчёт ставки выполнен по текущим итоговым очкам таблицы.</small></p>')}
...
server.listen(PORT,()=>console.log('FRISH v1.0.0 DIALOG RULES UPDATE on',PORT));
