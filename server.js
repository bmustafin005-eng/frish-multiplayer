...
function chooseSeatCard(r,i,idx){if(r.phase!=='seat_draw'||i!==r.seatTurn)return;const c=r.seatCards[Number(idx)];if(!c||c.taken)return;c.taken=true;r.seatChoices[i]=c.suit;addLog(r,`${r.players[i].name} вытянул ${c.suit}`);r.seatTurn++;emit(r);if(r.seatTurn>=4)return setTimeout(()=>finalizeSeats(r),700);autoSeatBots(r)}
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v1.2.0 GAMEPLAY UX FIX</title><style>
...
/* v1.2.0 compact table: keep all laid terets visible without vertical scrolling */
...
/* v1.2.0 — selected BLACK PREMIUM visual */
...
.seatDrawCard.taken{opacity:.18}.premiumBackMini{font-size:30px}
...
.dragGhost{position:fixed!important;pointer-events:none!important;z-index:99999!important;transform:translate(-50%,-60%) scale(1.08);box-shadow:0 10px 28px #000b!important}
...
let handOrder=[];function orderedHand(){if(!S)return[];const m=new Map(S.myHand.map(c=>[c.id,c]));const out=[];handOrder.forEach(id=>{if(m.has(id)){out.push(m.get(id));m.delete(id)}});for(const c of S.myHand){if(m.has(c.id)){out.push(c);m.delete(c.id)}}handOrder=out.map(c=>c.id);return out}function installDrag(){
 const h=q('#hand'),table=q('#melds');let dragId=null,pid=null,sx=0,sy=0,moved=false,previewIndex=null,ghost=null;
 const ghostMove=(x,y)=>{if(!ghost){const src=h.querySelector('[data-id="'+dragId+'"]');if(src){ghost=src.cloneNode(true);ghost.classList.add('dragGhost');document.body.appendChild(ghost)}}if(ghost){ghost.style.left=x+'px';ghost.style.top=y+'px'}};
 const ghostClear=()=>{if(ghost){ghost.remove();ghost=null}};
 ...
 h.querySelectorAll('.card[data-id]').forEach(el=>{
   el.onclick=null;
   el.onpointerdown=e=>{e.preventDefault();pid=e.pointerId;dragId=Number(el.dataset.id);sx=e.clientX;sy=e.clientY;moved=false;previewIndex=null;};
   el.onpointermove=e=>{if(pid!==e.pointerId)return;if(!moved&&Math.hypot(e.clientX-sx,e.clientY-sy)>7)moved=true;if(moved){ghostMove(e.clientX,e.clientY);preview(e.clientX,e.clientY)}};
   el.onpointerup=e=>{if(pid!==e.pointerId)return;const z=hit(e.clientX,e.clientY),ids=selectedIds();
     ...
     ghostClear();clearPreview();pid=null;dragId=null;previewIndex=null;render()
   };
   el.onpointercancel=()=>{ghostClear();clearPreview();pid=null;dragId=null;previewIndex=null;render()}
 })
...
server.listen(PORT,()=>console.log('FRISH v1.2.0 GAMEPLAY UX FIX on',PORT));
