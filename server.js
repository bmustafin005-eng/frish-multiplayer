...
function newRoom(hostName,sid){return {code:code(),players:[{name:hostName||'Игорь',sid,bot:false},null,null,null],started:false,dealNo:1,mult:2,dealer:0,cutter:3,phase:'lobby',stock:[],svetka:null,discard:[],hands:[[],[],[],[]],melds:[],turn:0,drawn:false,log:['Комната создана'],cutTimer:null,ran:[false,false,false,false],allDeclared:[false,false,false,false],cardSaid:[false,false,false,false],frishVote:null,scoreHistory:[],turnMeldPoints:[0,0,0,0],runReady:[false,false,false,false],turnSerial:0,runClaims:[null,null,null,null],ironClaims:[]}}
...
function startDeal(r){clearTimeout(r.cutTimer);r.dealer=(r.dealNo-1)%4;r.cutter=(r.dealer+3)%4;r.phase='cut';r.stock=deck();r.svetka=null;r.discard=[];r.hands=[[],[],[],[]];r.melds=[];r.runClaims=[null,null,null,null];r.ironClaims=[];r.turn=r.dealer;r.drawn=false;addLog(r,`${r.players[r.cutter].name} снимает колоду`);emit(r);r.cutTimer=setTimeout(()=>completeCut(r,Math.floor(r.stock.length/2),true),7000);if(r.players[r.cutter].bot)setTimeout(()=>completeCut(r,Math.floor(r.stock.length*(.35+Math.random()*.3)),true),900)}
...
function applyZastrel(r,offender,reason){
 const pts=20*r.mult;
 addLog(r,`ЗАСТРЕЛ: ${r.players[offender].name} +${pts} · ${reason}`);
 r.phase='zastrel';
 emit(r);
 const sameDeal=r.dealNo;
 r.mult+=1;
 setTimeout(()=>{if(r.dealNo===sameDeal)startDeal(r)},1400);
}
...
io.on('connection',socket=>{
 ...
 socket.on('discard',id=>{const [r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn)return;const dealerOpening=i===r.dealer&&r.hands[i].length===15&&!r.discard.length;if(!r.drawn&&!dealerOpening)return;const k=r.hands[i].findIndex(c=>c.id===id);if(k<0)return;const c=r.hands[i].splice(k,1)[0];r.discard.push(c);
 ...
 addLog(r,`${r.players[i].name} сбросил ${cardText(c)}`);endTurn(r)});
 ...
 socket.on('checkBadMeld',meldId=>{const[r,caller]=findRoomBySocket(socket);if(!r)return;
   ...
   if(m.validation&&m.validation.valid)return applyZastrel(r,caller,'ложная проверка: терец был правильным');
   return applyZastrel(r,m.owner,'неправильный терец');
 });
 ...
 socket.on('claimIron',({meldId,cardId})=>{const[r,caller]=findRoomBySocket(socket);if(!r)return;
   ...
   addLog(r,`${r.players[caller].name} заявил ЖЕЛЕЗКУ — ${cardText(c)}`);
   emit(r);
 });
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v0.8.2 CHECKS</title><style>
...
function openBadMelds(){
 let rows=S.melds.map((m,n)=>'<button class="badMeldBtn" data-mid="'+m.id+'">Терец '+(n+1)+' · '+S.players[m.owner].name+'</button>').join('<br>');
...
function openIronMelds(){
 let rows=S.melds.map((m,n)=>'<button class="ironMeldBtn" data-mid="'+m.id+'">Терец '+(n+1)+' · '+S.players[m.owner].name+'</button>').join('<br>');
