...
io.on('connection',socket=>{
 ...
 socket.on('sayCard',()=>{const[r,i]=findRoomBySocket(socket);if(!r)return;if(r.hands[i].length!==1)return socket.emit('errorMsg','КАРТА объявляется, когда в руке осталась ровно одна карта.');r.cardSaid[i]=true;if(!r.cardWarning[i])r.cardWarning[i]={dueSerial:r.turnSerial+4,said:true};else r.cardWarning[i].said=true;addLog(r,`${r.players[i].name} — КАРТА`);notice(r,`${r.players[i].name} — КАРТА`,5000);emit(r)});
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v1.1.1 CARD TABLE FIX</title><style>
...
/* v1.1.1 compact table: keep all laid terets visible without vertical scrolling */
...
/* v1.1.1 — selected BLACK PREMIUM visual */
...
server.listen(PORT,()=>console.log('FRISH v1.1.1 CARD TABLE FIX on',PORT));
