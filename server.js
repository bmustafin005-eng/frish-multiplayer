...
function publicState(r,viewer){return {code:r.code,players:r.players.map((p,i)=>p?{name:p.name,bot:p.bot,count:r.hands[i]?.length||0}:null),started:r.started,dealNo:r.dealNo,mult:r.mult,dealer:r.dealer,cutter:r.cutter,phase:r.phase,svetka:r.svetka,discard:r.discard.at(-1)||null,stockCount:r.stock.length,melds:r.melds,turn:r.turn,drawn:r.drawn,myIndex:viewer,myHand:viewer>=0?r.hands[viewer]:[],log:r.log.slice(-8),ran:r.ran,runReady:r.runReady,allDeclared:r.allDeclared,allFinisher:r.allFinisher,allDeadline:r.allDeadline}}
...
io.on('connection',socket=>{
 ...
 if(r.allDeclared[i]&&r.hands[i].length===0){
   ...
   r.reportTimer=setTimeout(()=>{
     ...
     setTimeout(()=>{r.dealNo++;startDeal(r)},3500);
   },15000);
   return;
 }
 endTurn(r)});
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v0.8.4 DRAG</title><style>
...
server.listen(PORT,()=>console.log('FRISH v0.8.4 DRAG on',PORT));
