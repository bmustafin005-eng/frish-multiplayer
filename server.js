...
function endTurn(r){const i=r.turn;if(r.allDeclared[i])return applyZastrel(r,i,'неправильное ВСЕ');r.takenDiscardId[i]=null;r.turn=(i+1)%4;r.drawn=false;beginTurn(r)}
...
io.on('connection',socket=>{
 ...
 socket.on('declareAll',()=>{const[r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn||r.allDeclared[i])return;
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v0.9.5 ALL STRICT FINISH</title><style>
...
/* v0.9.5 compact table: keep all laid terets visible without vertical scrolling */
...
server.listen(PORT,()=>console.log('FRISH v0.9.5 ALL STRICT FINISH on',PORT));
