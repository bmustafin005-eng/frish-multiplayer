...
function startStockRebuild(r){
 ...
 addLog(r,`Колода закончилась. ${r.players[r.dealer].name} перемешал карты; ${r.players[r.cutter].name} снимает колоду.`);
...
function beginTurn(r){buryDue(r);r.turnSerial++;r.turnMeldPoints[r.turn]=0;r.runReady[r.turn]=false;emit(r);if(r.players[r.turn].bot)setTimeout(()=>botTurn(r),650)}
...
function finishOrdinaryDeal(r,finisher){
 const delta=[0,0,0,0];
 delta[finisher]=(r.ran[finisher]?-5:-10)*r.mult;
...
function beginFinishReport(r,i,kind='ordinary'){
 ...
 addLog(r,`${r.players[i].name} закончил${kind==='pairs'?' ПАРАМИ':kind==='suit'?' МАСТЬЮ':''}. 15 секунд на ПРОВЕРКУ.`);emit(r);
...
io.on('connection',socket=>{
 ...
 if(r.allDeclared[i]&&r.pairFinish[i]&&r.pairFinish[i].count===7&&r.hands[i].length===1){const c=r.hands[i].splice(k,1)[0];r.discard.push(c);r.takenDiscardId[i]=null;r.reclaimedJokerId[i]=null;return beginFinishReport(r,i,'pairs');}
 ...
 socket.on('declareAll',()=>{const[r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn||r.allDeclared[i])return;r.allDeclared[i]=true;addLog(r,`${r.players[i].name} — ВСЕ`);notice(r,`${r.players[i].name} — ВСЕ`,5000);emit(r); });
 ...
 const m=r.melds.find(x=>String(x.id)===String(meldId));if(!m)return;const jok=m.cards.find(isJ),real=m.cards.filter(c=>!isJ(c));
 ...
 const cards=ids.map(id=>r.hands[i].find(c=>c.id===id)).filter(Boolean);if(cards.length!==14||cards.some(isJ)||!cards.every(c=>c.suit===cards[0].suit))return socket.emit('errorMsg','Выберите 14 карт одной масти.');
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v0.9.7 MOBILE SPECIAL FINISH</title><style>
...
/* v0.9.7 compact table: keep all laid terets visible without vertical scrolling */
...
const socket=io();let S=null,sel=new Set(),extendMode=false,extendTarget=null,reclaimMode=false,reclaimTarget=null;const q=s=>document.querySelector(s);function createRoom(){socket.emit('create',q('#name').value)}function joinRoom(){socket.emit('join',{name:q('#name').value,code:q('#code').value})}q('#createBtn').addEventListener('click',createRoom);q('#joinBtn').addEventListener('click',joinRoom);socket.on('joined',x=>{q('#roomCode').textContent=x.code;q('#lobby').classList.add('hidden');q('#room').classList.remove('hidden')});socket.on('errorMsg',alert);socket.on('state',s=>{S=s;render()});function ct(c){return !c?'':c.rank==='JOKER'?'J\\nO\\nK\\nE\\nR':c.rank+c.suit}function ce(c,click=true){let d=document.createElement('div');d.className='card '+((c&&['♥','♦'].includes(c.suit))?'red':'');if(c&&c.rank==='JOKER'){d.textContent=ct(c);d.style.whiteSpace='pre-line';d.style.lineHeight='1.02';d.style.fontSize='12px';d.style.color=c.suit==='R'?'#c71919':'#111'}else if(c){const a=document.createElement('span'),b=document.createElement('span');a.className='corner tl';b.className='corner br';a.textContent=c.rank+c.suit;b.textContent=c.rank+c.suit;d.append(a,b)}if(c&&click){d.onclick=()=>{sel.has(c.id)?sel.delete(c.id):sel.add(c.id);render()};if(sel.has(c.id))d.classList.add('selected')}return d}function render(){if(!S)return;const sn=q('#screenNotice');if(S.screenNotice&&S.screenNotice.text){sn.textContent=S.screenNotice.text;sn.classList.remove('hidden')}else{sn.textContent='';sn.classList.add('hidden')};q('#roomCode').textContent=S.code;q('#players').innerHTML=S.players.map((p,i)=>(i+1)+'. '+(p?p.name+(p.bot?' · БОТ':''):'свободно')).join('<br>');q('#start').style.display=(!S.started&&S.myIndex===0)?'inline-block':'none';if(!S.started)return;q('#game').classList.remove('hidden');q('#meta').textContent='Раздача '+S.dealNo+' · ×'+S.mult+' · раздаёт '+S.players[S.dealer].name;q('#phase').textContent=(S.phase==='cut'||S.phase==='rebuild_cut')?'Снимает '+S.players[S.cutter].name:(S.phase==='finish_report'?'15 СЕКУНД НА ПРОВЕРКУ':'Ход: '+S.players[S.turn].name);
 ...
 else {ab.textContent='';ab.classList.add('hidden')};q('#cutUI').classList.toggle('hidden',!(S.phase==='cut'||S.phase==='rebuild_cut'));q('#playUI').classList.remove('hidden');q('#cutText').textContent=S.players[S.cutter].name+' снимает колоду';q('#seats').innerHTML='';const relClass=['s3','s1','s0','s2'];for(let rel=0;rel<4;rel++){const i=(S.myIndex+rel)%4,p=S.players[i];let d=document.createElement('div');d.className='seat '+relClass[rel]+(S.turn===i&&S.phase==='turn'?' turn':'');d.textContent=p.name+' · '+p.count+' карт';q('#seats').appendChild(d)}q('#svetka').innerHTML='';if(S.svetka){let x=ce(S.svetka,false);x.classList.add('sv');q('#svetka').appendChild(x)}q('#discard').innerHTML='';if(S.discard)q('#discard').appendChild(ce(S.discard,false));q('#stockN').textContent=S.stockCount;q('#hand').innerHTML='';orderedHand().forEach(c=>{let el=ce(c,true);el.dataset.id=c.id;q('#hand').appendChild(el)});installDrag();const mine=S.phase==='turn'&&S.myIndex===S.turn;q('#stockBtn').disabled=!mine||S.drawn;q('#discardTakeBtn').disabled=!mine||S.drawn||!S.discard;q('#extendBtn').textContent=extendMode?'ВЫБЕРИТЕ ТЕРЕЦ':'ПОДЛОЖИТЬ';q('#extendBtn').disabled=!(S.ran&&S.ran[S.myIndex])&&!(S.allDeclared&&S.allDeclared[S.myIndex]);q('#pairsBtn').disabled=!(S.allDeclared&&S.allDeclared[S.myIndex]);q('#suitBtn').disabled=!(S.allDeclared&&S.allDeclared[S.myIndex]);q('#pairSvetkaBtn').disabled=!(S.allDeclared&&S.allDeclared[S.myIndex]);q('#pairsBtn').style.display=(S.allDeclared&&S.allDeclared[S.myIndex])?'block':'none';q('#pairSvetkaBtn').style.display=(S.allDeclared&&S.allDeclared[S.myIndex])?'block':'none';q('#suitBtn').style.display=(S.allDeclared&&S.allDeclared[S.myIndex])?'block':'none';q('#reclaimBtn').disabled=!(S.phase==='turn'&&S.turn===S.myIndex&&((S.ran&&S.ran[S.myIndex])||(S.allDeclared&&S.allDeclared[S.myIndex])));q('#reclaimBtn').style.display=((S.ran&&S.ran[S.myIndex])||(S.allDeclared&&S.allDeclared[S.myIndex]))?'block':'none';q('#melds').innerHTML='';S.melds.forEach(m=>{let d=document.createElement('div');d.className='meld'+(m.buryOnTurnOf!==null?' bury':'')+(String(extendTarget)===String(m.id)?' targetMeld':'')+(String(S.highlightMeldId)===String(m.id)?' zastrelHit':'');d.onclick=()=>{if(reclaimMode){reclaimTarget=m.id;confirmReclaim();return}if(!extendMode)return;extendTarget=m.id;confirmExtend()};m.cards.forEach(c=>{let x=ce(c,false);x.dataset.cardId=c.id;x.dataset.meldId=m.id;d.appendChild(x)});q('#melds').appendChild(d)});q('#log').textContent=S.log.join('\\n')}
q('#cutdeck').addEventListener('pointerup',e=>{if(!S||!(S.phase==='cut'||S.phase==='rebuild_cut')||S.myIndex!==S.cutter)return;let r=e.currentTarget.getBoundingClientRect(),p=Math.round(((e.clientY-r.top)/r.height)*80)+13;socket.emit('cut',p)});function drawStock(){socket.emit('drawStock')}function drawDiscard(){socket.emit('drawDiscard')}function toggleMore(){q('#moreActions').classList.toggle('hidden')}function layPair(){if(sel.size!==2)return alert('Выберите 2 карты пары.');socket.emit('layPair',orderedHand().filter(c=>sel.has(c.id)).map(c=>c.id));sel.clear()}
function layPairSvetka(){if(sel.size!==1)return alert('Выберите 1 карту для пары со светкой.');socket.emit('layPairWithSvetka',[...sel][0]);sel.clear()}
function finishSuit(){if(sel.size!==14)return alert('Выберите 14 карт одной масти.');socket.emit('finishSuit',orderedHand().filter(c=>sel.has(c.id)).map(c=>c.id));sel.clear()}
...
function layMeld(){if(sel.size<3)return alert('Выберите минимум 3 карты.');socket.emit('meld',orderedHand().filter(c=>sel.has(c.id)).map(c=>c.id));sel.clear()}function discardSelected(){if(sel.size!==1)return alert('Выберите одну карту.');socket.emit('discard',[...sel][0]);sel.clear()}function startExtend(){if(!(S.ran&&S.ran[S.myIndex])&&!(S.allDeclared&&S.allDeclared[S.myIndex]))return;if(!sel.size)return;extendMode=true;extendTarget=null;render()}function confirmExtend(){if(!extendMode||extendTarget==null)return;socket.emit('extendMeld',{meldId:extendTarget,ids:orderedHand().filter(c=>sel.has(c.id)).map(c=>c.id)});sel.clear();extendMode=false;extendTarget=null} 
let handOrder=[];function orderedHand(){if(!S)return[];const m=new Map(S.myHand.map(c=>[c.id,c]));const out=[];handOrder.forEach(id=>{if(m.has(id)){out.push(m.get(id));m.delete(id)}});for(const c of S.myHand){if(m.has(c.id)){out.push(c);m.delete(c.id)}}handOrder=out.map(c=>c.id);return out}function installDrag(){
 const h=q('#hand');let dragId=null,pid=null,sx=0,sy=0,moved=false,previewIndex=null;
 const clearPreview=()=>h.querySelectorAll('.dropBefore,.dropAfter,.dragging').forEach(x=>x.classList.remove('dropBefore','dropAfter','dragging'));
 const preview=(x,y)=>{
   ...
   const ids=handOrder.filter(id=>String(id)!==String(dragId));const target=Number(best.el.dataset.id),base=ids.findIndex(id=>String(id)===String(target));
 ...
 h.querySelectorAll('.card[data-id]').forEach(el=>{
   ...
   el.onpointerup=e=>{if(pid!==e.pointerId)return;
     if(moved&&previewIndex!==null){const ids=handOrder.filter(id=>String(id)!==String(dragId));ids.splice(Math.min(previewIndex,ids.length),0,dragId);handOrder=ids}
...
}function declareAll(){socket.emit('declareAll')}function sayCard(){socket.emit('sayCard')}function proposeFrish(){socket.emit('frishPropose')}function openCheck(){
...
function toggleScore(){let names=S.players.map(p=>p.name);let rows='';for(let d=1;d<=16;d++){const h=(S.scoreHistory||[]).find(x=>x.dealNo===d);const current=d===S.dealNo;const mult=h?h.mult:(current?S.dealDisplayMult:([1,6,11,16].includes(d)?2:1));const crosses=mult>1?' <span class="multCrosses">'+Array(mult-1).fill('×').join('')+'</span>':'';rows+='<tr'+(current?' style="font-weight:900;background:#fff4c2"':'')+'><td>'+d+crosses+'</td>'+names.map((_,i)=>'<td>'+(h&&h.totals?h.totals[i]:(current&&S.cumulative?S.cumulative[i]+S.dealAccum[i]:'—'))+'</td>').join('')+'</tr>'}showModal('<div style="display:flex;justify-content:space-between;align-items:center"><h2>ТАБЛИЦА</h2><button onclick="closeModal()">ЗАКРЫТЬ</button></div><table class="scoreOverlayTable"><tr><th>Раздача</th>'+names.map(n=>'<th>'+n+'</th>').join('')+'</tr>'+rows+'</table>')}function showModal(html){const m=q('#modal');m.className='overlay';m.innerHTML='<div class="modalBox" style="position:relative"><button class="modalClose" onclick="closeModal()" aria-label="Закрыть">×</button>'+html+'</div>'}function closeModal(){const m=q('#modal');m.className='hidden';m.innerHTML=''}
</script></body></html>`;

app.get('/',(req,res)=>res.type('html').send(HTML));
server.listen(PORT,()=>console.log('FRISH v0.9.7 MOBILE SPECIAL FINISH on',PORT));
