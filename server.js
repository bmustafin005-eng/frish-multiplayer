...
function newRoom(hostName,sid){return {code:code(),players:[{name:hostName||'Игорь',sid,bot:false},null,null,null],started:false,dealNo:1,mult:2,dealer:0,cutter:3,phase:'lobby',stock:[],svetka:null,discard:[],hands:[[],[],[],[]],melds:[],turn:0,drawn:false,log:['Комната создана'],cutTimer:null,ran:[false,false,false,false],allDeclared:[false,false,false,false],cardSaid:[false,false,false,false],frishVote:null,scoreHistory:[],turnMeldPoints:[0,0,0,0],runReady:[false,false,false,false],turnSerial:0}}
function cardText(c){return isJ(c)?'J':c.rank+c.suit}
function publicState(r,viewer){return {code:r.code,players:r.players.map((p,i)=>p?{name:p.name,bot:p.bot,count:r.hands[i]?.length||0}:null),started:r.started,dealNo:r.dealNo,mult:r.mult,dealer:r.dealer,cutter:r.cutter,phase:r.phase,svetka:r.svetka,discard:r.discard.at(-1)||null,stockCount:r.stock.length,melds:r.melds,turn:r.turn,drawn:r.drawn,myIndex:viewer,myHand:viewer>=0?r.hands[viewer]:[],log:r.log.slice(-8),ran:r.ran,runReady:r.runReady}}
...
io.on('connection',socket=>{
 ...
 socket.on('discard',id=>{const [r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn)return;const dealerOpening=i===r.dealer&&r.hands[i].length===15&&!r.discard.length;if(!r.drawn&&!dealerOpening)return;const k=r.hands[i].findIndex(c=>c.id===id);if(k<0)return;const c=r.hands[i].splice(k,1)[0];r.discard.push(c);if(r.runReady[i])r.ran[i]=true;addLog(r,`${r.players[i].name} сбросил ${cardText(c)}`);endTurn(r)});
 socket.on('meld',ids=>{const [r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn||(!r.drawn&&!(i===r.dealer&&r.hands[i].length===15&&!r.discard.length)))return;
   ...
   const validation=validateMeldCanonical(r,cs,mode);
   r.hands[i]=r.hands[i].filter(c=>!ids.includes(c.id));
   const m={id:Date.now()+Math.random(),owner:i,cards:cs,buryOnTurnOf:null,validation,laidAtTurnSerial:r.turnSerial,reportUntilTurnSerial:r.turnSerial+4,inactive:false};
   r.melds.push(m);
   if(validation.valid){
 ...
 socket.on('extendMeld',({meldId,ids})=>{const [r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn||!(r.ran[i]||r.runReady[i]))return socket.emit('errorMsg','Подложки доступны после набранных 51 или после побега.');ids=[...new Set((ids||[]).map(Number))];if(!ids.length||ids.length>4)return socket.emit('errorMsg','Выберите карты для подложки.');const m=r.melds.find(x=>String(x.id)===String(meldId));if(!m)return socket.emit('errorMsg','Выберите терец на столе.');const cs=ids.map(id=>r.hands[i].find(c=>c.id===id)).filter(Boolean);if(cs.length!==ids.length)return;const combined=[...m.cards,...cs];const validation=validateMeldCanonical(r,combined,r.allDeclared[i]?'ALL_FINISH':'NORMAL');if(!validation.valid)return socket.emit('errorMsg','Эти карты нельзя подложить к выбранному терцу.');r.hands[i]=r.hands[i].filter(c=>!ids.includes(c.id));m.cards=combined;m.validation=validation;if(m.cards.length===4&&m.cards.filter(isJ).length<=1&&m.cards.filter(c=>!isJ(c)).every(c=>c.rank===m.cards.find(x=>!isJ(x)).rank))m.buryOnTurnOf=i;addLog(r,`${r.players[i].name} сделал подложку`);emit(r)});
 socket.on('declareAll',()=>{const[r,i]=findRoomBySocket(socket);if(r&&r.phase==='turn'&&i===r.turn){r.allDeclared[i]=true;addLog(r,`${r.players[i].name} — ВСЕ`);emit(r)}});
 socket.on('sayCard',()=>{const[r,i]=findRoomBySocket(socket);if(r&&r.hands[i].length===1&&!r.cardSaid[i]){r.cardSaid[i]=true;addLog(r,`${r.players[i].name} — КАРТА`);emit(r)}});
 ...
 socket.on('checkZastrel',meldId=>{const[r,i]=findRoomBySocket(socket);if(!r)return;const m=r.melds.find(x=>String(x.id)===String(meldId));if(!m)return socket.emit('errorMsg','Выберите терец для проверки.');if(r.turnSerial>m.reportUntilTurnSerial)return socket.emit('errorMsg','Срок проверки этого действия уже истёк.');if(m.validation&&m.validation.valid){addLog(r,`${r.players[i].name}: ложная ПРОВЕРКА → ЗАСТРЕЛ заявителю`)}else{addLog(r,`${r.players[i].name}: ПРОВЕРКА подтверждена → ЗАСТРЕЛ ${r.players[m.owner].name}`)}emit(r)});
 socket.on('checkIron',()=>{const[r,i]=findRoomBySocket(socket);if(r){addLog(r,`${r.players[i].name} открыл ПРОВЕРКА → ЖЕЛЕЗКА`);emit(r)}});
