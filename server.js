...
function endTurn(r){r.takenDiscardId[r.turn]=null;r.turn=(r.turn+1)%4;r.drawn=false;beginTurn(r)}
...
io.on('connection',socket=>{
 ...
 socket.on('discard',id=>{const [r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn)return;if(r.takenDiscardId[i]!=null){const bad=r.takenDiscardId[i];r.takenDiscardId[i]=null;return applyZastrel(r,i,'карта из сброса не использована');}const dealerOpening=i===r.dealer&&r.hands[i].length===15&&!r.discard.length;if(!r.drawn&&!dealerOpening)return;const k=r.hands[i].findIndex(c=>c.id===id);if(k<0)return;const c=r.hands[i].splice(k,1)[0];r.discard.push(c);
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v0.9.4 JOKER RANK BURY FIX</title><style>
...
/* v0.9.4 compact table: keep all laid terets visible without vertical scrolling */
...
server.listen(PORT,()=>console.log('FRISH v0.9.4 JOKER RANK BURY FIX on',PORT));
