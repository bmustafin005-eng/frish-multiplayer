...
function publicState(r,viewer){return {code:r.code,players:r.players.map((p,i)=>p?{name:p.name,bot:p.bot,count:r.hands[i]?.length||0}:null),started:r.started,dealNo:r.dealNo,mult:r.mult,dealer:r.dealer,cutter:r.cutter,phase:r.phase,svetka:r.svetka,discard:r.discard.at(-1)||null,stockCount:r.stock.length,melds:r.melds,turn:r.turn,drawn:r.drawn,myIndex:viewer,myHand:viewer>=0?r.hands[viewer]:[],log:r.log.slice(-8),ran:r.ran,runReady:r.runReady,allDeclared:r.allDeclared,allFinisher:r.allFinisher,allDeadline:r.allDeadline,screenNotice:r.screenNotice,scoreHistory:r.scoreHistory,cumulative:r.cumulative,dealAccum:r.dealAccum,highlightMeldId:r.highlightMeldId,dealDisplayMult:r.dealDisplayMult,reclaimedJokerPending:viewer>=0?!!r.reclaimedJokerId[viewer]:false}}
...
io.on('connection',socket=>{
 ...
 socket.on('finishPairs',()=>{const[r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn||!r.allDeclared[i])return;
 const pf=findPairsFinish(r.hands[i]);if(!pf)return socket.emit('errorMsg','Нужны 7 пар и одна карта на финальный сброс.');
 ...
 r.hands[i]=[];r.discard.push(pf.discard);r.takenDiscardId[i]=null;r.reclaimedJokerId[i]=null;notice(r,`${r.players[i].name} — ВСЕ · ПАРЫ`,5000);beginFinishReport(r,i,'pairs');});
 socket.on('finishSuit',()=>{const[r,i]=findRoomBySocket(socket);if(!r||r.phase!=='turn'||i!==r.turn||!r.allDeclared[i])return;
 const sf=findSuitFinish(r.hands[i]);if(!sf)return socket.emit('errorMsg','Нужны 14 карт одной масти и одна карта на финальный сброс.');
 r.melds.push({id:Date.now()+Math.random(),owner:i,cards:sf.cards,validation:{valid:true,type:'SUIT_FINISH'},laidAtTurnSerial:r.turnSerial,reportUntilTurnSerial:r.turnSerial+4,inactive:false,buryOnTurnOf:null});
 r.hands[i]=[];r.discard.push(sf.discard);r.takenDiscardId[i]=null;r.reclaimedJokerId[i]=null;notice(r,`${r.players[i].name} — ВСЕ · МАСТЬ`,5000);beginFinishReport(r,i,'suit');});
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v0.9.6 SPECIAL FINISH JOKER RECLAIM</title><style>
...
/* v0.9.6 compact table: keep all laid terets visible without vertical scrolling */
...
server.listen(PORT,()=>console.log('FRISH v0.9.6 SPECIAL FINISH JOKER RECLAIM on',PORT));
