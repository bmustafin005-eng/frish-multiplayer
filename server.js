...
function publicState(r,viewer){return {code:r.code,players:r.players.map((p,i)=>p?{name:p.name,bot:p.bot,count:r.hands[i]?.length||0}:null),started:r.started,dealNo:r.dealNo,mult:r.mult,dealer:r.dealer,cutter:r.cutter,phase:r.phase,svetka:r.svetka,discard:r.discard.at(-1)||null,stockCount:r.stock.length,melds:r.melds,turn:r.turn,drawn:r.drawn,myIndex:viewer,myHand:viewer>=0?r.hands[viewer]:[],log:r.log.slice(-8),ran:r.ran,runReady:r.runReady,allDeclared:r.allDeclared,allFinisher:r.allFinisher,allDeadline:r.allDeadline,screenNotice:r.screenNotice,scoreHistory:r.scoreHistory,cumulative:r.cumulative,dealAccum:r.dealAccum,highlightMeldId:r.highlightMeldId,dealDisplayMult:r.dealDisplayMult,reclaimedJokerPending:viewer>=0?!!r.reclaimedJokerId[viewer]:false,pairFinish:viewer>=0?r.pairFinish[viewer]:null,allFromHand:r.allFromHand}}
...
const HTML=`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ФРИШ Multiplayer v0.9.9 DRAG ALL SCORE FIX</title><style>
...
/* v0.9.9 compact table: keep all laid terets visible without vertical scrolling */
...
server.listen(PORT,()=>console.log('FRISH v0.9.9 DRAG ALL SCORE FIX on',PORT));
